...
def main(args):
    ...
    # ---------- test seen data ---------- #
    testSeen_f, testSeen_l = np.array(
        hf.get('testSeen_f')), np.array(hf.get('testSeen_l'))
    # ---------- test unseen data ---------- #
    ...
    netCLF = Myclassifier(seen_embeddings.size(1), len(
        myloader.allclasses_name)).to(args.device)
    ...
    best_Epoch, best_S, best_U, best_H = 0, 0.0, 0.0, 0.0
    # ======================================== Config dataloader ======================================== #
    # ---------- trainloader ---------- #
    ...
    for epoch in range(1, args.nepoch+1):
        ...
        new_trainloader = DataLoader(
            new_trainset, batch_size=args.batch_size, shuffle=False)
        record_miniEpoch = [0.0, 0.0, 0.0]
        for clf_epoch in range(args.clf_nepoch):
            ...
            if H > record_miniEpoch[2]:
                record_miniEpoch[:3] = [S, U, H]

        early_break += 1
        if record_miniEpoch[2] > best_H:
            best_Epoch = epoch
            best_S, best_U, best_H = record_miniEpoch[:]
            # if args.save_model:
            #     torch.save(netE.state_dict(), f"{outlogPath}/{args.dataset}_netE_bestGZSL.pth")
            #     torch.save(netD.state_dict(), f"{outlogPath}/{args.dataset}_netD_bestGZSL.pth")
            #     torch.save(netCLF.state_dict(), f"{outlogPath}/{args.dataset}_netCLF_bestGZSL.pth")
            #     print('BestGZSL models saved!')
            early_break = 0

        dictNow = {'S': record_miniEpoch[0], 'U': record_miniEpoch[1], 'H': record_miniEpoch[2]}

        print("Performance => Epoch:{}; S:{:.6f}; U:{:.6f}; H:{:.6f}".format(
            epoch, dictNow['S'], dictNow['U'], dictNow['H']))
        print('Best GZSL => Epoch:{}; S:{:.6f}; U:{:.6f}; H:{:.6f}'.format(
            best_Epoch, best_S, best_U, best_H))
        print("Early break: ", early_break)

        logger.info("Performance => Epoch:{}; S:{:.6f}; U:{:.6f}; H:{:.6f}".format(
            epoch, dictNow['S'], dictNow['U'], dictNow['H']))
        logger.info('Best GZSL => Epoch:{}; S:{:.6f}; U:{:.6f}; H:{:.6f}'.format(
            best_Epoch, best_S, best_U, best_H))
        logger.info("-"*50)
